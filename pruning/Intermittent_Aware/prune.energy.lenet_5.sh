...
# 30.4% pruned -- 99.19%
python main.py $COMMON_FLAGS --prune $PRUNE_METHOD --stage 0 --group $GROUP_SIZE \
...
# 51.6% pruned -- 99.17%
python main.py $COMMON_FLAGS --prune $PRUNE_METHOD --stage 1 --group $GROUP_SIZE\
...
python main.py $COMMON_FLAGS --prune $PRUNE_METHOD --stage 5 --group $GROUP_SIZE\
	...
	$MY_DEBUG --sa
'''
python main.py $COMMON_FLAGS --prune $PRUNE_METHOD --stage 6 --group $GROUP_SIZE\
	--pretrained saved_models/$PRUNE_METHOD/LeNet_5.prune.group_size5.5.pth.tar \
	--lr 0.01 --lr-epochs 20 \
	--candidates-pruning-ratios $CANDIDATES_PRUNING_RATIOS \
	$MY_DEBUG --sa
'''
